...
# how many species are lost per sample group when excluding bacteria
# that are found only in nematode samples and not PC:


# ---- 1. SPEICHERE die Tabelle vor dem Filtern (falls noch nicht getan) ----
species_before <- read.csv("species_table_clean.csv", row.names = 1)

# Entferne wie vorher die unerwünschten Nematoden-Gruppen
cols_to_remove <- grep("Nothacrobeles|Panagrolaimus", colnames(species_before), value = TRUE)
species_before <- species_before[, !colnames(species_before) %in% cols_to_remove]


# ---- 2. Definiere die PC-Spalten ----
pc_cols <- c("PC_sample_A_PC1", "PC_sample_E_PC4",
             "PC_sample_F_PC5", "PC_sample_G_PC3",
             "PC_sample_M_PC2")

# ---- 3. Bakterien, die in mindestens einem PC vorkommen ----
bact_in_pc <- rownames(species_before)[
  rowSums(species_before[, pc_cols, drop = FALSE], na.rm = TRUE) > 0
]

# ---- 4. Tabelle nach PC gefiltert ----
species_after <- species_before[rownames(species_before) %in% bact_in_pc, , drop = FALSE]


# ---- 5. Samplegruppen zuweisen ----
sample_groups <- sapply(colnames(species_before), function(x) {
...
# Lass nur echte Nematodengruppen übrig (keine PC)
nematode_samples <- names(sample_groups)[sample_groups != "PC"]
nematode_groups  <- sample_groups[sample_groups != "PC"]


# ---- 6. FUNKTION: Anzahl Taxa pro Gruppe berechnen ----
taxa_in_group <- function(df, group_samples, group) {
  cols <- group_samples[group_samples == group]
  rows_present <- rowSums(df[, names(cols), drop = FALSE] > 0, na.rm = TRUE) > 0
  sum(rows_present)
}

groups <- unique(nematode_groups)

# ---- 7. Anzahl Taxa vorher / nachher ----
taxa_before <- sapply(groups, function(g) taxa_in_group(species_before, nematode_groups, g))
taxa_after  <- sapply(groups, function(g) taxa_in_group(species_after,  nematode_groups, g))

# ---- 8. Prozentverlust durch PC-Filter ----
percent_loss <- 100 * (taxa_before - taxa_after) / taxa_before


# ---- 5. Samplegruppen zuweisen (robust) ----
patterns <- list(
  "A. emmatus"      = "Acrobeles_emmatus",
  "A. mariannae"    = "Acrobeles_mariannae",
  "Acrobeles sp."   = "Acrobeles_sp",
  "Acrobeloides"    = "Acrobeloides",
  "Cervidellus"     = "Cervidellus",
  "Mesorhabditis"   = "Mesorhabditis",
  "Stegelleta"      = "Stegelleta",
  "Stegelletina"    = "Stegelletina",
  "PC"              = "PC"
)

sample_groups <- sapply(colnames(species_before), function(s) {
  grp <- NA_character_
  for (name in names(patterns)) {
    if (grepl(patterns[[name]], s)) {
      grp <- name
      break
    }
  }
  grp
}, USE.NAMES = TRUE)  # Named character vector

# ---- 6. Nur Nematoden-Samples ----
nematode_samples <- names(sample_groups)[sample_groups != "PC" & !is.na(sample_groups)]
nematode_groups  <- sample_groups[sample_groups != "PC" & !is.na(sample_groups)]
groups <- unique(nematode_groups)  # Jetzt korrekt

# ---- 7. Anzahl Taxa pro Gruppe berechnen ----
taxa_in_group <- function(df, group_samples, group) {
  cols <- names(group_samples[group_samples == group])
  if (length(cols) == 0) return(0)
  rows_present <- rowSums(df[, cols, drop = FALSE] > 0, na.rm = TRUE) > 0
  sum(rows_present)
}
...
# ---- 8. Prozentverlust ----
percent_loss <- 100 * (taxa_before - taxa_after) / taxa_before
percent_loss[is.nan(percent_loss)] <- 0
...
loss_table <- data.frame(
  ...
  Lost = taxa_before - taxa_after,
  Percent_lost = round(percent_loss, 2)
)
